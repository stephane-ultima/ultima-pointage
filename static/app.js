...
function getFribourgHolidays(year) {
  const E=getEaster(year);
  const d=(base,n)=>{const r=new Date(base);r.setDate(r.getDate()+n);return r.toISOString().split('T')[0];};
  const f=(m,day)=>`${year}-${String(m).padStart(2,'0')}-${String(day).padStart(2,'0')}`;
  return {[f(1,1)]:'Nouvel An',[d(E,-2)]:'Vendredi Saint',[d(E,1)]:'Lundi de P�ques',
    [d(E,39)]:'Ascension',[d(E,50)]:'Lundi de Pentec�te',[d(E,60)]:'F�te-Dieu',
    [f(8,1)]:'F�te nationale',[f(8,15)]:'Assomption',[f(11,1)]:'Toussaint',
    [f(12,8)]:'Immacul�e Conception',[f(12,25)]:'No�l',[f(12,26)]:'Saint-�tienne'};
}
function getWeekDates(week,year) {
  const jan4=new Date(year,0,4),dow=jan4.getDay()||7;
  const mon1=new Date(jan4.getTime()-(dow-1)*86400000);
  const monday=new Date(mon1.getTime()+(week-1)*7*86400000);
  return Array.from({length:5},(_,i)=>{const d=new Date(monday.getTime()+i*86400000);return d.toISOString().split('T')[0];});
}
...
function SideNav({ current, role, onNav, isOpen, onClose, user }) {
  ...
  const tabs = isManager
    ...
    : [
        { id: 'today',    icon: 'clock',    label: 'Pointage' },
...
function TodayScreen({ user, meData, onRefresh }) {
  ...
  const [actType, setActType] = React.useState('WORK_SITE');
  ...
  const [editStartHm, setEditStartHm] = React.useState('');
  const [editEndHm, setEditEndHm] = React.useState('');
  const [editActType, setEditActType] = React.useState('WORK_SITE');
  ...
  React.useEffect(() => {
    if (!active) return;
    const iv = setInterval(() => setElapsed(fmt.elapsed(active.started_at)), 1000);
    setElapsed(fmt.elapsed(active.started_at));
  ...
  const startEntry = async () => {
    ...
    try {
      await api.post('/time-entries/start', { activity_type: actType });
      setShowStart(false);
      await loadData();
    } catch(e) { setActError(e.message||'Erreur'); }
  ...
  const submitDay = async () => {
    setSubmitting(true);
    try {
      await api.post('/time-entries/submit-day', { date: viewDate });
      await loadData();
  ...
  if (loading) return React.createElement('div',{className:'flex items-center justify-center h-64'},React.createElement(Spinner,{size:'lg'}));

  return (
    <div className="max-w-lg mx-auto pb-24">
      <div className="flex items-center justify-between px-4 py-3 bg-white border-b sticky top-0 z-10">
        <button onClick={prevDay} className="p-2 rounded-full hover:bg-slate-100 text-slate-600 text-xl font-bold">&lt;</button>
        <div className="text-center">
          <div className="font-semibold text-slate-800 capitalize">{dateLabel}</div>
          {isToday
            ? <div className="text-xs text-emerald-600 font-medium">Aujourd'hui</div>
            : <div className="text-xs text-slate-500">{viewDate}</div>}
        </div>
        <button onClick={nextDay} disabled={isToday}
          className={"p-2 rounded-full text-xl font-bold " + (isToday ? "text-slate-300 cursor-default" : "hover:bg-slate-100 text-slate-600")}>
          &gt;
        </button>
      </div>

      <div className="p-4 space-y-4">
        {hasReturned && returnNote && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-sm text-red-700">
            <span className="font-semibold">A corriger :</span> {returnNote}
          </div>
        )}
            ...
            {active ? (
              <div className="text-center space-y-3">
                <div className="text-xs font-medium text-slate-500 uppercase tracking-wide">
                  {active.activity_type === 'BREAK' ? 'Pause en cours' : 'Activite en cours'}
                </div>
                <div className="text-5xl font-mono font-bold text-slate-800 tabular-nums">{elapsed}</div>
                <div className="text-sm text-slate-500">
                  {active.activity_type === 'WORK_SITE' ? 'Chantier'
                    : active.activity_type === 'TRAVEL' ? 'Trajet'
                    : active.activity_type === 'BREAK' ? 'Pause'
                    : active.activity_type}
                </div>
                <button onClick={stopEntry} className="w-full bg-red-500 hover:bg-red-600 text-white font-semibold py-3 rounded-xl transition-colors">
                  Arreter
                </button>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                <div className="text-center text-sm text-slate-500">{pct}% de l'objectif</div>
                {!hasPending && (
                  <button onClick={() => setShowStart(true)} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-xl transition-colors">
        ...
        {!isToday && (
          <div className="bg-white rounded-xl shadow-sm border p-4">
            ...
            <div className="text-center text-sm text-slate-500 mt-2">{pct}% de l'objectif</div>
          </div>
        )}

        {user.employee_type === 'MONTEUR' && (
          <div className={"rounded-xl border p-3 flex items-center gap-3 " + (mealOk?"bg-emerald-50 border-emerald-200":"bg-slate-50 border-slate-200")}>
            <span className={"text-xl font-bold " + (mealOk?"text-emerald-600":"text-slate-400")}>{mealOk?"OK":"--"}</span>
            <div>
              <div className="text-sm font-semibold text-slate-700">Indemnite repas</div>
              <div className="text-xs text-slate-500">{mealOk?"CHF 20.00 acquis":"Requiert plus de 4h de travail"}</div>
            </div>
          </div>
        )}
        ...
        {done.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border divide-y overflow-hidden">
            {done.map(e => {
              ...
              const typeLabel = e.activity_type==='WORK_SITE'?'Chantier':e.activity_type==='TRAVEL'?'Trajet':e.activity_type==='BREAK'?'Pause':e.activity_type==='OFFICE'?'Bureau':e.activity_type;
              return (
                <div key={e.id} className="p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm font-medium text-slate-700">{typeLabel}</div>
                      <div className="text-xs text-slate-500">{toHm(e.started_at)||''} – {toHm(e.ended_at)||''}</div>
                    </div>
                  ...
                  {e.status==='RETURNED' && e.note && (
                    <div className="mt-1 text-xs text-red-600 italic">{e.note}</div>
                  )}
                  {e.status==='RETURNED' && (
                    <button onClick={() => openEdit(e)}
                      className="mt-2 w-full text-xs bg-orange-500 hover:bg-orange-600 text-white font-semibold py-2 rounded-lg transition-colors">
                      Corriger cette entr�e
                    </button>
                  )}
                </div>
              );
      ...
      {showStart && (
        <div className="fixed inset-0 bg-black/50 flex items-end z-50" onClick={() => setShowStart(false)}>
          <div className="bg-white rounded-t-2xl p-6 w-full space-y-4" onClick={e => e.stopPropagation()}>
            <h3 className="font-semibold text-slate-800 text-lg">Nouvelle activite</h3>
            {actError && <div className="text-red-600 text-sm bg-red-50 border border-red-200 rounded-lg p-2">{actError}</div>}
            <div className="grid grid-cols-2 gap-3">
              {[{v:'WORK_SITE',label:'Chantier'},{v:'TRAVEL',label:'Trajet'},{v:'BREAK',label:'Pause'},{v:'OFFICE',label:'Bureau'}].map(opt => (
                <button key={opt.v} onClick={() => setActType(opt.v)}
                  className={"py-3 rounded-xl border-2 font-medium text-sm transition-colors " + (actType===opt.v?"border-blue-600 bg-blue-50 text-blue-700":"border-slate-200 text-slate-600 hover:bg-slate-50")}>
                  {opt.label}
                </button>
              ))}
            </div>
            <button onClick={startEntry} disabled={actLoading} className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-semibold py-3 rounded-xl transition-colors">
              {actLoading ? 'Demarrage...' : 'Demarrer'}
            </button>
            <button onClick={() => { setShowStart(false); setActError(''); }} className="w-full py-2 text-slate-500 text-sm">Annuler</button>
          </div>
        </div>
      )}

      {editEntry && (
        <div className="fixed inset-0 bg-black/50 flex items-end z-50" onClick={() => !editLoading && setEditEntry(null)}>
          <div className="bg-white rounded-t-2xl p-6 w-full space-y-4" onClick={ev => ev.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-slate-800 text-lg">Corriger l'entr�e</h3>
              <button onClick={() => setEditEntry(null)} className="text-slate-400 hover:text-slate-600 text-2xl leading-none">�</button>
            </div>
            {editEntry.note && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-sm text-red-700">
                <span className="font-semibold">Note du responsable :</span> {editEntry.note}
              </div>
            )}
            {editError && <div className="text-red-600 text-sm bg-red-50 border border-red-200 rounded-lg p-2">{editError}</div>}
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">D�but</label>
                <input type="time" value={editStartHm} onChange={ev => setEditStartHm(ev.target.value)}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1">Fin</label>
                <input type="time" value={editEndHm} onChange={ev => setEditEndHm(ev.target.value)}
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-2">Type d'activit�</label>
              <div className="grid grid-cols-2 gap-2">
                {[{v:'WORK_SITE',label:'Chantier'},{v:'TRAVEL',label:'Trajet'},{v:'BREAK',label:'Pause'},{v:'OFFICE',label:'Bureau'}].map(opt => (
                  <button key={opt.v} onClick={() => setEditActType(opt.v)}
                    className={"py-2 rounded-xl border-2 font-medium text-sm transition-colors " + (editActType===opt.v?"border-blue-600 bg-blue-50 text-blue-700":"border-slate-200 text-slate-600 hover:bg-slate-50")}>
                    {opt.label}
                  </button>
                ))}
              </div>
            ...
            <button onClick={saveEdit} disabled={editLoading}
              ...
              {editLoading ? 'Enregistrement...' : 'Soumettre la correction'}
            </button>
          </div>
        </div>
      )}
...
function HoursScreen({ user }) {
  ...
  const [loading, setLoading] = useState(true);
  const [{ week, year }, setWeekYear] = React.useState(() => getISOWeek());
  ...
  const openEdit = (e) => {
    const toHm = ts => ts ? new Date(ts * 1000).toLocaleTimeString('fr-CH', { hour: '2-digit', minute: '2-digit' }) : '';
    setEditEntry(e);
    setEditStartHm(toHm(e.started_at));
    setEditEndHm(toHm(e.ended_at));
    setEditActType(e.activity_type || 'WORK_SITE');
    setEditError('');
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      await api.patch('/time-entries/' + editEntry.id + '/edit', {
        started_hm: editStartHm,
        ended_hm: editEndHm,
        activity_type: editActType,
      });
      setEditEntry(null);
      await loadData();
  ...
  const exportPDF = async () => {
    ...
    const W = 210, mg = 15;
    let y = 20;
    const actLabel = t => ({ WORK_SITE: 'Chantier', TRAVEL: 'D�placement', TRAINING: 'Formation', OFFICE: 'Bureau', OTHER: 'Autre' }[t] || t);
    const toHm = ts => ts ? new Date(ts * 1000).toLocaleTimeString('fr-CH', { hour: '2-digit', minute: '2-digit' }) : '–';
    const fmtDur = m => { const h = Math.floor(m / 60); const mn = m % 60; return h + 'h' + String(mn).padStart(2, '0'); };
    const fmtDate = d => new Date(d + 'T12:00:00').toLocaleDateString('fr-CH', { weekday: 'long', day: 'numeric', month: 'long' });
    doc.setFontSize(18); doc.setFont('helvetica', 'bold');
    doc.text('Ultima Interior SA', mg, y); y += 8;
    doc.setFontSize(11); doc.setFont('helvetica', 'normal');
    doc.text((user.first_name || '') + ' ' + (user.last_name || ''), mg, y); y += 6;
    doc.text('Semaine ' + week + ' – ' + year, mg, y); y += 8;
    doc.setDrawColor(180, 180, 180); doc.line(mg, y, W - mg, y); y += 7;
    for (const wd of weekDates) {
      if (y > 260) { doc.addPage(); y = 20; }
      const holiday = holidays[wd];
      const dayPair = byDay.find(([d]) => d === wd);
      const dayEntries = dayPair ? dayPair[1].filter(e => e.activity_type !== 'MEAL') : [];
      const dayWork = dayEntries.reduce((s, e) => s + (e.duration_min || 0), 0);
      doc.setFontSize(10); doc.setFont('helvetica', 'bold'); doc.setTextColor(30, 30, 30);
      const dayLabel = fmtDate(wd) + (holiday ? ' — ' + holiday : '');
      doc.text(dayLabel, mg, y);
      if (dayWork > 0) { doc.setFont('helvetica', 'normal'); doc.text(fmtDur(dayWork), W - mg, y, { align: 'right' }); }
      else if (holiday && dayEntries.length === 0) { doc.setFont('helvetica', 'italic'); doc.setTextColor(120, 70, 160); doc.text('Jour f�ri�', W - mg, y, { align: 'right' }); doc.setTextColor(30, 30, 30); }
      y += 5;
      if (dayEntries.length > 0) {
        doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.setTextColor(70, 70, 70);
        for (const e of dayEntries) {
          if (y > 270) { doc.addPage(); y = 20; }
          doc.text('  ' + toHm(e.started_at) + ' – ' + toHm(e.ended_at) + '   ' + actLabel(e.activity_type), mg, y);
          if (e.duration_min) doc.text(fmtDur(e.duration_min), W - mg, y, { align: 'right' });
          y += 4.5;
        }
        doc.setTextColor(30, 30, 30);
      } else if (!holiday) {
        doc.setFontSize(9); doc.setFont('helvetica', 'italic'); doc.setTextColor(160, 160, 160);
        doc.text('  Pas d’enregistrement', mg, y); doc.setTextColor(30, 30, 30); y += 4.5;
      }
      y += 2; doc.setDrawColor(220, 220, 220); doc.line(mg, y, W - mg, y); y += 5;
    }
    if (y > 268) { doc.addPage(); y = 20; }
    doc.setFontSize(11); doc.setFont('helvetica', 'bold'); doc.setTextColor(30, 30, 30);
    doc.text('Total semaine', mg, y);
    doc.text(fmtDur(totalWork), W - mg, y, { align: 'right' }); y += 5;
    doc.setFontSize(9); doc.setFont('helvetica', 'normal'); doc.setTextColor(100, 100, 100);
    doc.text('Objectif : ' + fmtDur(weeklyTarget), W - mg, y, { align: 'right' });
    doc.setFontSize(8); doc.setTextColor(160, 160, 160);
    doc.text('Ultima Pointage', mg, 288);
    doc.text(new Date().toLocaleDateString('fr-CH'), W - mg, 288, { align: 'right' });
    doc.save('pointage-S' + week + '-' + year + '.pdf');
  };
  ...
  const weeklyTarget = (user.weekly_target_h || 42) * 60;
  const holidays = React.useMemo(() => Object.assign({},
    getFribourgHolidays(year-1), getFribourgHolidays(year), getFribourgHolidays(year+1)
  ), [year]);
  const weekDates = React.useMemo(() => getWeekDates(week, year), [week, year]);
  ...
  return (
    <div className="space-y-4 pb-6 max-w-lg">
      ...
      <Card className="p-4">
        ...
        <button onClick={exportPDF} title="Exporter PDF" className="w-10 h-10 rounded-xl bg-blue-50 hover:bg-blue-100 flex items-center justify-center transition-colors">
          <Ic name="download" size={18} className="text-blue-500" />
        </button>
        </div>
      ...
      {loading ? (
        ...
        byDay.map(([day, dayEntries]) => {
          ...
          return (
            <Card key={day}>
              <button onClick={() => toggleDay(day)} className="w-full px-4 py-3 border-b border-slate-50 flex items-center justify-between text-left">
              <div>
                <span className="text-sm font-semibold text-slate-700 capitalize">{fmt.weekday(day)}</span>
                <span className="text-xs text-slate-400 ml-2">{new Date(day + 'T12:00:00').toLocaleDateString('fr-CH', { day: 'numeric', month: 'short' })}</span>
              </div>
              <div className="flex items-center gap-2">
                ...
                {holidays[day] && <span className="text-xs bg-violet-100 text-violet-700 border border-violet-200 px-2 py-0.5 rounded-full font-medium">{holidays[day]}</span>}
                <span className="text-sm font-bold text-slate-700">{fmt.duration(dayWork)}</span>
                <Ic name={expandedDays.has(day) ? 'chevron-up' : 'chevron-down'} size={14} className="text-slate-400" />
              </div>
            </button>
            {expandedDays.has(day) && (<div className="divide-y divide-slate-50">
                {dayEntries.map(e => {
                  const at = ACTIVITY_TYPES[e.activity_type] || ACTIVITY_TYPES.OTHER;
                  return (
      ...
      {weekDates.filter(day => holidays[day] && !byDay.some(([d]) => d === day)).map(day => (
        <Card key={day} className="p-4 border border-violet-200 bg-violet-50">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-semibold text-slate-700 capitalize">{fmt.weekday(day)}</span>
              <span className="text-xs text-slate-400 ml-2">{new Date(day + 'T12:00:00').toLocaleDateString('fr-CH', { day: 'numeric', month: 'short' })}</span>
            </div>
            <span className="text-xs bg-violet-100 text-violet-700 border border-violet-200 px-2 py-0.5 rounded-full font-medium">{holidays[day]}</span>
          </div>
        </Card>
      ))}
      {!loading && entries.length > 0 && (
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-slate-700">Total semaine</span>
            <div className="text-right">
              <span className="text-base font-bold text-slate-800">{fmt.duration(totalWork)}</span>
              <span className="text-xs text-slate-400 ml-1">/ {fmt.duration(weeklyTarget)}</span>
            </div>
          </div>
          {totalWork > 0 && (
            <div className="mt-2 w-full bg-slate-100 rounded-full h-1.5">
              <div className={totalWork >= weeklyTarget ? 'h-1.5 rounded-full bg-emerald-500' : totalWork >= weeklyTarget * 0.6 ? 'h-1.5 rounded-full bg-blue-500' : 'h-1.5 rounded-full bg-amber-400'}
                style={{ width: Math.min(100, (totalWork / weeklyTarget) * 100) + '%' }} />
            </div>
          )}
        </Card>
      )}
      {editEntry && (
        <div className="fixed inset-0 bg-black/40 flex items-end z-50" onClick={() => setEditEntry(null)}>
          <div className="bg-white rounded-t-2xl w-full p-5 space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-slate-800">Corriger l'entr�e</h3>
              <button onClick={() => setEditEntry(null)} className="text-slate-400"><Ic name="x" size={20} /></button>
            </div>
            {editEntry.manager_note && (
              <div className="bg-orange-50 border border-orange-200 rounded-xl p-3 text-sm text-orange-700">
                <span className="font-medium">Note : </span>{editEntry.manager_note}
              </div>
            )}
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-slate-500 font-medium">D�but</label>
                <input type="time" value={editStartHm} onChange={e => setEditStartHm(e.target.value)}
              ...
              <div>
                ...
                <input type="time" value={editEndHm} onChange={e => setEditEndHm(e.target.value)}
                  className="w-full mt-1 px-3 py-2 border border-slate-200 rounded-xl text-sm" />
              </div>
            </div>
            <div>
              <label className="text-xs text-slate-500 font-medium">Type d'activit�</label>
              <select value={editActType} onChange={e => setEditActType(e.target.value)}
